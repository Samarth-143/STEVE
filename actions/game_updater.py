...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.steve.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.steve.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "STEVE_GameUpdater", "/F"],
            capture_output=True, text=True
        )
        return ("Scheduled update cancelled."
                if result.returncode == 0 else "No scheduled update found.")
    if is_mac():
        plist_path = Path.home() / "Library" / "LaunchAgents" / "com.steve.gameupdater.plist"
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "STEVE_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True
        )
        if result.returncode != 0:
